<template>
  <div class="core-tabs" v-hotkey="keymap">
    ...
    <confirmation-modal :id="confirmModalId">
      ...
      <template #message> You will lose unsaved changes </template>
    </confirmation-modal>
...
export default Vue.extend({
    ...
    rootBindings() {
      return [
        ...
        { event: AppEvent.switchUserKeymap, handler: this.switchUserKeymap },
        {
          ...
          handler: this.pasteAsNewRowsWrongTabCheck,
        },
      ];
    },
    lastTab() {
  ...
  methods: {
    ...
    async importSqlFiles(importConfig: { paths: string[]; parentId: number }) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          const text = await this.$util.send("file/readSqlFile", {
            path: file.path,
          });
          if (text) {
    ...
    async handlePromptQueryExport(query) {
      ...
      const fullQuery = await this.$store.dispatch(
    ...
    pasteAsNewRowsWrongTabCheck() {
      ...
      if (this.activeTab?.tabType !== "table") {
        this.$noty.error(
          "Paste as new rows is only available in a table's Data tab"
        );
      }
    },
    openTableBuilder() {
    ...
    async closeAll() {
      ...
      if (unsavedTabs.length > 0) {
        const confirmed = await this.$confirm(
          "关闭全部标签？",
          `有 ${unsavedTabs.length} 个未保存的标签，确定要关闭吗？`
        );
        if (!confirmed) return;
      }
    ...
    async closeToRight(tab: TransportOpenTab) {
      ...
      if (unsavedTabs.length > 0) {
        const confirmed = await this.$confirm(
          "关闭右侧的标签？",
          `有 ${unsavedTabs.length} 个未保存的标签将被关闭，确定吗？`
        );
        if (!confirmed) return;
      }
