<template>
  ...
  >
    <div class="top-panel" ref="topPanel">
      <div v-if="querySelectionError" class="query-parser-error">
        <div class="alert alert-warning alert-small">
          <i class="material-icons">error_outline</i>
          <div class="alert-body">当前选中运行不可用</div>
          <div class="btn-group">
            <a
      ...
      <div class="no-content" v-if="remoteDeleted">
        <div class="alert alert-danger">
          <i class="material-icons">error_outline</i>
          <div class="alert-body">此查询已被其他人删除，无法再编辑。</div>
          <a @click.prevent="close" class="btn btn-flat">关闭标签</a>
        </div>
      </div>
      <component
        :is="editorComponent"
          ...
          <div v-if="canManageTransactions">
                ...
                <x-button
                  ...
                  v-tooltip="
                    getCommitModeVTooltip({
                      title: '自动提交模式',
                      description: '这是默认的工作方式，无需额外处理。',
                    })
                  "
                >
                  <span class="togglebutton-content">
                    {{ !isManualCommit ? "自动提交" : "自动" }}
                  </span>
                </x-button>
                <x-button
                  :toggled="isManualCommit"
            ...
            <transition name="fade-swap">
              <div
                ...
                v-tooltip="{
                  ...getCommitModeVTooltip({
                    title: `<i class='material-icons'>commit</i><span>事务进行中</span>`,
                    description: '一旦提交或回滚，事务即结束。',
                    learnMoreLink:
                      'https://docs.beekeeperstudio.io/user_guide/sql_editor/manual-transaction-management',
        ...
        <div class="actions primary-actions btn-group">
          ...
          <x-buttons class="" v-tooltip="runButtonTooltip">
            <x-button
              ...
              :disabled="
                runButtonDisabled || (primaryIsCurrent && runCurrentDisabled)
              "
            >
            ...
            >
              ...
              <x-menu>
                ...
                <x-menuitem
                  ...
                  :disabled="
                    disableRunToFile || (primaryIsCurrent && runCurrentDisabled)
                  "
                >
                ...
                <x-menuitem
                  ...
                  :disabled="
                    disableRunToFile || (primaryIsTab && runCurrentDisabled)
                  "
                >
    ...
    <div class="bottom-panel" ref="bottomPanel">
      ...
      <div class="message" v-else-if="result">
        <div class="alert alert-info">
          ...
          <span
            >查询 {{ selectedResult + 1 }}/{{ results.length }}：无结果。影响
            {{
              result.affectedRows || 0
            }}
            行。可在左下角选择框查看更多的查询结果。</span
          >
        </div>
      </div>
    ...
    <portal to="modals">
      ...
      >
        <form v-kbd-trap="true" v-if="query" @submit.prevent="saveQuery">
          <div class="dialog-content">
            <div class="dialog-c-title">保存的查询名称</div>
...
export default {
  ...
  computed: {
    ...
    rootBindings() {
      return [
        {
          event: AppEvent.openQueryEditHistory,
          handler: this.handleOpenQueryEditHistory,
        },
    ...
    query() {
      return (
        this.fullQuery ||
        this.savedQueries.find((q) => q.id === this.tab.queryId) ||
        this.blankQuery
      );
    },
    queryId() {
    ...
    runningText() {
      return `正在运行 ${this.runningType}（${this.runningCount} 个查询）`;
    },
    hasSelectedText() {
    ...
    autocompleteKeywordCasing() {
      const value = String(
        this.$bksConfig.ui.queryEditor?.autocomplete?.keywordCasing ?? ""
      ).toLowerCase();
      return ["preserve", "upper", "lower"].includes(value)
        ? value
        : "preserve";
    },
    autocompleteQuoteIdentifiers() {
      const value = String(
        this.$bksConfig.ui.queryEditor?.autocomplete?.quoteIdentifiers ?? ""
      ).toLowerCase();
    ...
    disableKeywordCompletion() {
      return this.$bksConfig.ui.queryEditor?.autocomplete
        ?.disableKeywordCompletion;
    },
  ...
  methods: {
    ...
    async submitCurrentQuery() {
      ...
      const { queries, error } = safelyIdentify(this.unsavedText, {
    ...
    async submitQuery(rawQuery, fromModal = false) {
      ...
      const { queries: identification, error } = safelyIdentify(rawQuery, {
    ...
    initializeQueries() {
      const { originalText, editorText } = resolveEditorText(
        this.tab,
        this.query?.text
      );
      if (editorText) {
        // The run methods should catch any errors, so we don't need to do that here
        const { queries } = safelyIdentify(editorText, {
          dialect: this.identifierDialect,
          paramTypes: this.paramTypes,
        });
        if (queries.length > 0) {
    ...
    openTroubleshooting() {
      window.main.openExternally(
        "https://docs.beekeeperstudio.io/support/troubleshooting/"
      );
    },
    copyQuerySelectionError() {
      this.$native.clipboard.writeText(
        this.querySelectionError?.stack ?? this.querySelectionError?.message
      );
    },
    startTimer() {
      this.elapsedTime = 0;
