...
protocol.registerSchemesAsPrivileged([
  { scheme: "plugin", privileges: { secure: true, standard: true } },
]);
...
async function initBasics() {
  ...
  if (settings.oracleInstantClient) {
    process.env[
      "LD_LIBRARY_PATH"
    ] = `${process.env.LD_LIBRARY_PATH}:${settings.oracleInstantClient.value}`;
  }

  const defaultChannel =
    settings.useBeta.defaultValue === "true" ? "beta" : "stable";
  ...
  if (platformInfo.parsedAppVersion.channel !== defaultChannel) {
    settings.useBeta.defaultValue =
      platformInfo.parsedAppVersion.channel === "beta" ? "true" : "false";
    log.debug(
      "Updating the default channel to",
      platformInfo.parsedAppVersion.channel
    );
...
app.on("window-all-closed", () => {
  ...
  // In development we keep the process alive after the window is closed so
  // the dev services (esbuild/vite) keep running and the window can be
  // reopened from the terminal by typing `o` (handled by the esbuild watcher).
  if (process.platform !== "darwin" && !platformInfo.isDevelopment) {
    app.quit();
  }
});
